...
```bash
# 检测并删除PK视频（默认直接删除）
python3 check_pk.py /path/to/dir

# 仅预览，不删除
python3 check_pk.py /path/to/dir --dry-run

```

### 工作原理

1. 使用 ffmpeg 从视频中均匀提取 2 帧截图
2. 通过图像分析检测分屏结构（PK场景特征）
3. 如果超过30%的截图检测到分屏结构，判定为PK视频并删除
